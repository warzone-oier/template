...
注意到若 $\mathbf{P}_i^c\le n<\mathbf{P}_i^{c+1}$，则 $\left\lfloor\dfrac{n}{\mathbf{P}_1^{c+1}}\right\rfloor=0$，因此
$$S_k(n)=S_{\mathbf{P}}(n)-S_{\mathbf{P}}(\mathbf{P}_{k-1})+\sum_{\small i\ge k\atop \mathbf{P}_i^2\le n}\sum_{\small c\ge 1\atop \mathbf{P}_i^{c+1}\le n}(f(\mathbf{P}_i^{c})S_{k+1}\left(\left\lfloor\dfrac{n}{\mathbf{P}_i^{c}}\right\rfloor\right)+f(\mathbf{P}_i^{c+1}))$$
