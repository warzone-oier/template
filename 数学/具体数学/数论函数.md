...
- $\mu(\displaystyle\prod_{i=1}^n p_i^{k_i})=0$（存在 $k_i\ge 2$）
...
- $(\operatorname{DGF} f)'=f(1)-\displaystyle\sum_{k=2}^\infty\dfrac{f(k)}{n^x}\ln k,\int(\operatorname{DGF}f)(x)\mathrm{d}x=f(1)-\sum_{k=2}^\infty\dfrac{f(k)}{k^x\ln k }$  
  实际操作中常用质因子次数和代替 $\ln$。  
